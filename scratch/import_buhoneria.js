...
async function importBuhoneria() {
  console.log('--- Importando Buhoneros desde XLSX ---');
  ...
  await supabase.from('taxpayers').delete().eq('commercial_category', 'BUHONERIA');
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (name && typeof name === 'string' && name.trim() !== '' && !name.toUpperCase().includes('CONTRIBUYENTE')) {
      ...
      const cleanCode = code ? String(code).trim() : `GEN-BUH-${i}`;
      const key = `${cleanName.toLowerCase()}-${cleanCode.toLowerCase()}`;

      if (!seenKeys.has(key)) {
        taxpayers.push({
          taxpayer_number: `${cleanCode}-BUH-${i}`,
          name: cleanName,
          type: 'NATURAL',
          status: 'ACTIVO',
          doc_id: `SD-BUH-${cleanCode}-${i}`,
          address: addr ? String(addr).trim() : 'ALMIRANTE',
          has_commercial_activity: true,
          commercial_category: 'BUHONERIA',
  ...
  console.log(`¡Importación de Buhonería Exitosa! (${taxpayers.length} registros)`);
}
