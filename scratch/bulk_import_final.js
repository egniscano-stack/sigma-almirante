...
async function importAll() {
  console.log('--- Iniciando Carga Masiva Almirante (FIXED UNIQUE) ---');
  ...
  for (const fileInfo of FILES_TO_IMPORT) {
    ...
    await supabase.from('taxpayers').delete().eq('commercial_category', fileInfo.category);
    ...
    for (let i = fileInfo.startRow; i < rows.length; i++) {
      ...
      if (name && typeof name === 'string' && name.trim() !== '' && !name.toUpperCase().includes('CONTRIBUYENTE')) {
        ...
        if (!seenKeys.has(key)) {
          taxpayers.push({
            // Temporary unique number to avoid constraint error
            taxpayer_number: `TEMP-${fileInfo.category.substring(0,3)}-${i}-${Date.now()}`, 
            name: cleanName,
            type: 'JURIDICA',
            status: 'ACTIVO',
  ...
  console.log('\n--- Regenerando Números de Contribuyente Oficiales (2026-X) ---');
  const { data: allTaxpayers, error: fetchError } = await supabase.from('taxpayers').select('id').order('name', { ascending: true });
  ...
  if (allTaxpayers) {
    console.log(`Total a procesar: ${allTaxpayers.length} registros.`);
