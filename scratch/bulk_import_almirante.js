...
async function importAll() {
  ...
  for (const fileInfo of FILES_TO_IMPORT) {
    ...
    for (let i = fileInfo.startRow; i < rows.length; i++) {
      ...
      if (name && typeof name === 'string' && name.trim() !== '' && !name.toUpperCase().includes('CONTRIBUYENTE')) {
        ...
        if (!seenKeys.has(key)) {
          taxpayers.push({
            taxpayer_number: '---', // Will be generated at the end
            name: cleanName,
            type: 'JURIDICA',
            status: 'ACTIVO',
  ...
  console.log('\n--- Regenerando Números de Contribuyente (2026-X) para todo el Catastro ---');
  const { data: allTaxpayers } = await supabase.from('taxpayers').select('id').order('name', { ascending: true });
  
  if (allTaxpayers) {
    console.log(`Actualizando ${allTaxpayers.length} registros...`);
    const chunkSize = 20;
